	...
	with open(data_filepath) as instream:
		...
		for line in instream:
			if line == '# vertices\n':
				...
				new_vertex = np.array(cast_list(line.split()[1:], float))
				vertices = np.append(vertices, new_vertex[np.newaxis,:], axis=0)
			elif output_type == 'edges' and line != '\n':
				new_edge = np.array(cast_list(line.split()[1:-2], int))
				edges = np.append(edges, new_edge[np.newaxis,:], axis=0)
			elif output_type == 'faces' and line != '\n':
				new_face = cast_list(line.split()[1:], int)
				faces = np.append(faces, [None])
				faces[-1] = new_face
	edges = edges-1
	for face_index, face in enumerate(faces):
		vert_face = np.zeros(len(face), dtype = int)
		for edge_index, edge in enumerate(face):
			if edge > 0:
				vert_face[edge_index] = edges[edge-1,0]
			else:
				vert_face[edge_index] = edges[-edge-1,1]
		faces[face_index] = vert_face
