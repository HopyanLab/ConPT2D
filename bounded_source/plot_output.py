...
def plot_output (result_file = Path('../bounded_output/results.csv'),
				 output_file = Path('../plots/bounded_output_plot.svg'),
				 return_ax = False):
	...
	ax.plot(np.ones_like(y_values)*3.85, y_values,
			color = 'black', linestyle = '--', marker = '')
		#	label = '3.813')
	x = np.linspace(3,4,1000)
	y = np.linspace(1e-3,1.7,1000)
	...
	test_point_F = [3.85, 0.08, 1.0, 1.0, 0.1]
#	test_point_f = [3.8/1.6, 3.8, 0.1]
	test = np.array([])
	...
	#				label = f'r = {current_r:1.1f}')
		params, covar = optimize.curve_fit(F, p0_param[r_param == current_r],
											  ...
											  p0 = test_point_F,
											  bounds = (0,24))
	#	params, covar = optimize.curve_fit(f, energy[r_param == current_r],
	#										  p0_param[r_param == current_r],
	#										  p0 = test_point_f,
	#										  bounds = (0,24))
		if current_r == 0.5:
			print(params)
		test = np.append(test,params[0])
		test_point_F = params
	#	test_point_f = params
		ax.plot(x,F(x,*params), color = colors[index], linestyle = '-')
	#	ax.plot(f(y,*params),y, color = colors[index], linestyle = '-')
	print(np.mean(test))
	print(np.std(test))
	ax.grid(True)
...
if __name__ == '__main__':
	...
	parser.add_argument('-o', '--outfile',
						nargs = 1,
						default = ['../plots/bounded_output_plot.svg'],
						type = str,
						required = False,
